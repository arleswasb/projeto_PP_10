 ...
 *
 * Compilação: gcc -o pi_critical -fopenmp pi_critical_compartilhado.c -lm
 * Execução:   ./pi_critical
 */
...
int main() {
    ...
    printf("Executando Versão: Contador Compartilhado + critical\n");
    ...
    #pragma omp parallel
    {
        ...
        for (long i = 0; i < NUM_PASSOS; i++) {
            ...
            if (x * x + y * y < 1.0) {
                // A seção crítica protege o bloco de código.
